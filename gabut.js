...
const vcard = 'BEGIN:VCARD\n' //card
            ...
            + 'ORG: Developer ᴀᴅɪ ᴏғғɪᴄɪᴀʟあ;\n' //nama creator

            + 'TEL;type=CELL;type=VOICE;waid=6289504585790:+62 895-0458-5790 \n' //nomor kamu
...
const premium = ["16672377823@s.whatsapp.net","6289504585790@s.whatsapp.net"]
		...
		try {
			...
			mess = {
				...
				only: {
					...
					daftarB: `*「 REGISTER 」* \nHalo Kak, Yuk Registrasi Dulu Soalnya Anda Belum Teregistrasi Dalam Database Bot\n\nKetik : ${prefix}register nama|umur\nContoh : ${prefix}register AdiOfficial|17`,
...
const bares = getLevelingLevel(sender)
		...
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*「 LINK GROUP DETECTOR 」*\n${sender.split("@")[0]} Maaf Anda Di Kick Dari Group Karena Mengirim Link!!!*`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 1000)
		setTimeout( () => {
...
/*********** FUNCTION ANTI BUCIN ***********/
		...
		if (messagesC.includes("#harusizintod")) return reply("izin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*「 BUCIN DETECTOR 」*\n${sender.split("@")[0]} Maaf Anda Di Kick Dari GROUP KARENA BUCIN*`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 1000)
		setTimeout( () => {
...
*╠➥ SUBSCRIBE メ*
*╠➥ https://bit.ly/2ON7iL6*
*╠➥ FREE REST API メ*
*╠➥ https://gratisancok.herokuapp.com/*
*╠❒ Total Fitur : 200 ++*
*╠❒ Total Chat : ${totalchat.length}*
*╠❒ Total block : ${blocked.length}*
*╠❒ Total Premium : ${prem.length}*
*╠❒ Total Pengguna : ${_registered.length}*
*╠❒ Bot Aktif Sejak : ${kyun(uptime)}*
*║*
*╠═══〖 UMUM 〗*
*║*
...
▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱
`
                    client.sendMessage(from, AdiGans, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "𝗧𝗘𝗥𝗩𝗘𝗥𝗜𝗙𝗜𝗞𝗔𝗦𝗜\n*YouTube: Adi Official 🌼*\n*Instagram: ff_patr1ck 🌼*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('src/logobot.jpg')} } }, caption: wew, pushname, prefix, getLevelingLevel, getLevelingXp, sender, reqXp, _registered, uangku, role, time })
					break
...
break
		...
		if (!isPrem) return reply(mess.only.premium)
                ...
                anu = await fetchJson(`https://gratisancok.herokuapp.com/api/yt-play/?kata=${play}&apikey=${ApikeyZailani}`,  {method: 'get'})
                if (anu.error) return reply(anu.error)
                komp3 = `*Hasil Pencarian*\n➽ *Judul* : ${anu.result.title}\n➽ *Upload* : ${anu.result.uploaded}\n➽ *Durasi* : ${anu.result.duration}\n➽ *Total View* : ${anu.result.total_view}\n➽ *Channel* : ${anu.result.channel}\n➽ *File Size* : ${anu.result.filesize}\nTunggu SEBENTAR AUDIO SEDANG DI KIRIM*`
		...
		if (anu.error) return reply(anu.error)
		komp3 = `「 *PLAY DOWNLOADER*」\n*Judul Lagu*: ${anu.result.title}\n*Durasi Video*: ${anu.result.duration}\n*Size*: ${anu.result.size}`
...
break
				...
				if (!isRegister) return reply(mess.only.daftarB)
					...
					teks = `*Nama bot* : ${me.name}\n*Author* : *Adi Official*\n*Nomor Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}\n*Public:* OFF\n*Total User Premium*: ${prem.length}\n*Total Chat* : ${totalchat.length}\n*Instagram* : *ff_patr1ck*\n*Github* : *github.com/COMINGSOON*\n*Youtube* : https://bit.ly/2ON7iL6\n*GROUP BOT*: http://bit.ly/Grupgabutbotv1`
			...
			case 'owner':
				...
				if (!isRegister) return reply(mess.only.daftarB)
                 ...
                 client.sendMessage(from, 'Kalau Mau Membuat BOT Kunjungi Channel ini Yahh',MessageType.text, { quoted: mek} )
                 
                 client.sendMessage(from, `https://youtube.com/channel/UCXzxFx9pitmYFLJo4nHrRPg Support Kami Ya Guys`,MessageType.text, { quoted: mek} )
...
if (!isRegister) return reply(mess.only.daftarB)
			...
			if (!isRegister) return reply(mess.only.daftarB)
				...
				if (!isRegister) return reply(mess.only.daftarB)
					...
					case 'register':
                                        ...
                                        if (umurUser > 30) return reply(`your age is too  old maximum 40 years`)

                                        if (umurUser < 13) return reply(`your age is too young minimum 12 years`)
			...
			case 'modeanime':
				...
				if (!isRegister) return reply(mess.only.daftarB)
					...
					if ((args[0]) === 'enable') {
						...
						reply('ᴋᴇᴛɪᴋ ᴇɴᴀʙʟᴇ ᴜɴᴛᴜᴋ ᴍᴇɴɢᴀᴋᴛɪғᴋᴀɴ\nᴋᴇᴛɪᴋ ᴅɪsᴀʙʟᴇ ᴜɴᴛᴜᴋ ᴍᴇɴᴏɴᴀᴋᴛɪғᴋᴀɴ')
					}
					break
					...
					if (Number(args[0]) === 1) {
						...
						if (isAntiLinkYoutube) return reply('anti link Youtube sudah aktif')
					...
					if (Number(args[0]) === 1) {
						...
						if (isAntiLinkTiktok) return reply('anti link Tiktok sudah aktif')

						antilinktiktod.push(from)

						fs.writeFileSync('./database/group/antilinktiktod.json', JSON.stringify(antilinktiktod))

						reply('Sukses mengaktifkan anti link Tiktok di group ini ✔️')

						client.sendMessage(from,`Perhatian kepada seluruh member anti link Tiktok aktif apabila anda mengirim link Tiktod anda akan di kick dari group`, text)

					} else if (Number(args[0]) === 0) {

						if (!isAntiLinkTiktok) return reply('Mode anti link Tiktok sudah disable')
						...
						reply('Sukes menonaktifkan anti link Tiktok di group ini ✔️')
					...
					if (Number(args[0]) === 1) {
						...
						if (isAntiLinkInstagram) return reply('anti link Instagram sudah aktif')
						...
						if (!isAntiLinkInstagram) return reply('Mode anti link Instagram sudah disable')
					...
					if (Number(args[0]) === 1) {
						...
						if (isAntiBucin) return reply('anti bucin sudah aktif')

						antibucin.push(from)

						fs.writeFileSync('./database/group/antibucin.json', JSON.stringify(antibucin))

						reply('Sukses mengaktifkan anti bucin di group ini ✔️')

						client.sendMessage(from,`Perhatian kepada seluruh member anti bucin aktif apabila anda uwuw uwuw an di grup anda akan di kick dari group`, text)

					} else if (Number(args[0]) === 0) {

						if (!isAntiBucin) return reply('Mode anti Bucin sudah disable')
						...
						reply('Sukes menonaktifkan anti bucin di group ini ✔️')
			...
			case 'attp2':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply(mess.only.benned)
					if (args.length < 1) return reply(`*Text Kosong*\n\n*CONTOH*\n*${prefix}attp AdiOfficial*`)
					teks = body.slice(6)
	...
	if (isBanned) return reply(mess.only.benned)
		...
		break
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `Ᏼ Ꭱ Ꮎ Ꭺ Ꭰ Ꮯ Ꭺ Ꮪ Ꭲ\n\n${body.slice(4)}`})
						}
                ...
                if (isBadWord) return reply('*fitur Anti Toxic sudah aktif sebelum nya*')
               ...
               //menu download Update Mzailani & Adi Official
				...
				case 'ytmp3':
					...
					if (anu.error) return reply(anu.error)
					teks = `*❏ Title* : ${anu.result.title}\n\n*AUDIO SEDANG DIKIRIMKAN, JANGAN SPAM YA SAYANG*`
					thumb = await getBuffer(anu.result.thumb)
					client.sendMessage(from, thumb, image, { quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result.result)
                ...
                infomp3 = `*Lagu Ditemukan!!!*\n*Judul* : ${data.result.result.judul}\n*Artist* : ${data.result.result.artist}\n*Album* : ${data.result.result.album}`
		...
		case 'zodiak':
			...
			teks `*Zodiak :* ${anu.result.zodiak}\n*Ramalan*: ${anu.result.ramalan}\n*Nomer Keberuntungan*: ${anu.result.nomorKeberuntungan}\n*Motivasi*: ${anu.result.motivasi}\n*Inspirasi*: ${anu.result.inspirasi}`
			client.sendMessage(from, teks, text, {quoted: mek})
			break
			case 'ramalhp':
				if (isBanned) return reply(mess.only.benned)    
				if (!isRegister) return reply(mess.only.daftarB)
				if (args.length < 1) return reply('Yg mau Di ramal Hp nya Siapa om?')
					kj = body.slice(12)
...
// SCRIPT 100% GRATISS
// SUBSCRIBE CHANNEL Adi Official BIAR TAU UPDATE FITUR TERBARU 🥰
